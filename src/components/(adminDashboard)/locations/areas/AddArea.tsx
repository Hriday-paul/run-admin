import { useAddAreaMutation, useAllDivisionsQuery, useDistrictsByDivisionQuery } from '@/redux/api/locations.api';
...
function AddArea() {
    ...
    return (
        <div>
            <Button size='large' type='primary' onClick={() => setOpen(true)}>Add Area</Button>
            <Modal
                open={open}
...
const AddAreaForm = () => {
    ...
    const divQuery: any = {};
    if (division) {
        divQuery.division = division;
    }
    const { data: districts, isLoading: districtload, isFetching : districtfetch, isSuccess: districtSuccess } = useDistrictsByDivisionQuery(divQuery);


    const [form] = Form.useForm();

    const [handleAdd, { isLoading: adLoading }] = useAddAreaMutation();
        ...
        >
            ...
            >
                <Select
                    ...
                    options={isSuccess ? districts?.data?.map(i => {
                        return { label: i?.name, value: i?.id }
                    }) : []}
                />
            </Form.Item>
